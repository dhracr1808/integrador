...
const Options = () => {
  const { user } = UseHookUser();
  return user.id ? <IsAuth /> : <ButonsAuth />;
};

const ButonsAuth = () => {
  const { home, status, toggleModal } = UseHookModal();

  const [type, setType] = useState(true);

  const showModal = (show) => {
    toggleModal(true);
    if (!show) return setType(false);
    setType(true);
  };

  return (
    <>
      <button onClick={() => showModal()} className="btn-loggin">
        Loggin
      </button>

      <button onClick={() => showModal(true)} className="btn-register">
        Register
      </button>
      {!home && status && <Modal content={type ? <Register /> : <Loggin />} />}
    </>
  );
};
